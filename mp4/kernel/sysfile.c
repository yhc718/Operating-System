...
uint64 sys_open(void)
{
    ...
    else
    {
        ...
        if (omode != O_NOACCESS) {
            // namex already follows symlinks in the *middle* of the path;
            // here we peel off any trailing symlink.
            while (ip->type == T_SYMLINK) {
                
                char linkbuf[MAXPATH+1];
                int len;

                // read link target
                ilock(ip);
                len = readi(ip, 0, (uint64)linkbuf, 0, MAXPATH);
                iunlock(ip);

                iput(ip);
                linkbuf[len] = '\0';
                // resolve the link target from the top again
                ip = namei(linkbuf);
        ...
        ilock(ip);
        if (ip->type == T_DIR && (omode != O_RDONLY && omode != O_NOACCESS && omode != O_FOLLOW))
        {
            iunlockput(ip);
            end_op();
            return -1;
    ...
    if (omode != O_NOACCESS && omode != O_FOLLOW) {
        if (omode == O_RDWR) {
            if ((ip->minor & M_ALL) != M_ALL) {
                iunlockput(ip);
                end_op();
                return -1;
            }
    ...
    f->ip = ip;
    f->readable = !(omode & O_WRONLY) && (omode != O_NOACCESS && omode != O_FOLLOW);
    f->writable = ((omode & O_WRONLY) || (omode & O_RDWR)) && (omode != O_NOACCESS && omode != O_FOLLOW);
...
uint64 sys_chmod(void)
{
    ...
    begin_op();
    if (argstr(0, path, MAXPATH) < 0 || argint(1, &add) < 0 || argint(2, &mask) < 0 || (ip = namei(path)) == 0)
    {
        end_op();
        return -1;
    }
...
uint64 sys_symlink(void)
{
    ...
    if (writei(ip, 0, (uint64)target, 0, len) != len) {
        // on failure, drop the inode and abort
        iunlockput(ip);
        end_op();
        return -1;
    }
