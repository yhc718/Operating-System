...
static struct inode *namex(char *path, int nameiparent, char *name)
{
    ...
    while ((path = skipelem(path, name)) != 0)
    {
        ...
        if (next->type == T_SYMLINK && *path != '\0') {
            ...
            // Read the link’s target path
            ilock(next);
            len = readi(next, 0, (uint64)linkbuf, 0, MAXPATH);
            iunlock(next);
            iput(next);
            linkbuf[len] = '\0';

            // Build a new path: <link target> + "/" + <remaining original path>
            char newpath[MAXPATH];
            safestrcpy(newpath, linkbuf, MAXPATH);
            char *p = newpath + strlen(newpath);
            *p = '/';
            p++;
            safestrcpy(p, path, MAXPATH - strlen(newpath));

            // restart resolution from the top
            iunlock(ip);
